    ...
    T: KVStore,
{
    default fn _push_message<M: Encode>(
        ...
        self._emit_event_message_queued(id, data.encode());
...
mod tests {
    ...
    fn test_push_and_pop_message() {
        ...
        let queue_index = contract._push_message(&message1).unwrap();
        ...
        let queue_index = contract._push_message(&message2).unwrap();
        assert_eq!(1, queue_index);
        assert_eq!(0, contract.get_queue_head().unwrap());
        assert_eq!(2, contract.get_queue_tail().unwrap());

        // get the first message
        let message_in_queue: Option<u128> = contract._get_message(0).unwrap();
        assert_eq!(
            message1,
        ...
        let message_in_queue: Option<u16> = contract._get_message(1).unwrap();
        assert_eq!(
            message2,
            message_in_queue.expect("we expect a message in the queue")
        );
